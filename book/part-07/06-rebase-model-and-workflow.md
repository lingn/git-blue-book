...
先理解 rebase 如何用新父提交重建对象，再按恢复引用、冲突、中止、验证和共享推送的顺序操作。交互式 todo 的具体动作见[交互式 rebase](05-interactive-rebase.md)。
...
本章讲普通 rebase 的可执行流程。需要修改提交顺序、拆分提交或批量执行命令时，进入[交互式 rebase](05-interactive-rebase.md)。
