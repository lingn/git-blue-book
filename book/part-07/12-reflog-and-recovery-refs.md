...
恢复决策不能只看 `git reflog` 是否能搜到一行。候选至少分成以下几种状态：

| reflog 线索 | 对象状态 | 结论 | 下一步 |
| --- | --- | --- | --- |
| 有 | `^{commit}` 可读 | 本地可恢复 | 先建立 recovery ref，再比较 tree 和父关系 |
| 有 | 对象缺失或损坏 | 只有坐标证据 | 冻结清理，转对象取证或向其他仓库索取对象 |
| 无 | `^{commit}` 可读 | 可能来自其他 ref、备份或外部记录 | 保存完整 OID，核对来源和签名，再建立临时引用 |
| 无 | 对象不可读 | 本地没有足够证据 | 不猜短哈希，不继续改写，进入取证流程 |

`git show` 能显示内容也不等于候选就是正确业务版本。恢复引用只是把候选保持可达，后续仍要比较目标 tree、父提交、提交范围、测试和共享系统记录。若候选来自外部仓库，先记录来源仓库、获取时间、传输方式和摘要，不能把“对象能 fetch 到”写成“对象来源可信”。
...
实验从仓库根目录执行，要求 Git、Bash 和 `mktemp` 可用；它使用隔离临时仓库，不读取当前项目的 refs、配置或工作区：

```bash
git --version
TMPDIR=/private/tmp bash scripts/verify-reset-reflog.sh
```
...
实验依次建立提交、执行 `soft`、`mixed`、`hard` 和路径形式的 reset，然后读取 `ORIG_HEAD` 与 HEAD reflog，为被移出正式分支的提交创建 recovery ref。预期状态是：reset 可能移动当前分支和 index/工作区，但 recovery ref 创建后候选重新由普通引用保持可达；脚本失败时返回非零并在退出时清理临时目录。不要把脚本中的 reset 或恢复分支命令替换成当前项目的真实分支名。

实验只证明本地 Git 的引用日志、`ORIG_HEAD`、对象可读性和恢复引用关系，不模拟 reflog 过期、对象清理、文件系统损坏、远程服务端保留、平台评审、CI、制品或其他 clone。真实事故中仍要保存原始 OID、命令时间、仓库/工作树位置和外部证据。
