...
状态机不是给工单换一组名字。每次转换都要检查与该阶段相应的证据，发现事实变化时允许回退，但不能覆盖旧记录：

| 状态 | 最低进入证据 | 会触发回退的变化 | 不能提前宣称 |
| --- | --- | --- | --- |
| `triaged` | 影响范围、严重度、IC、受影响组件和初始时间线 | 新增受影响资产、严重度上升或控制面失联 | 根因已确认 |
| `evidence_frozen` | 运行基线、制品/配置/schema、Git refs、采集摘要和访问范围 | 仍有写入、自动维护或采集缺口 | 现场完整无缺 |
| `mitigated` | 缓解动作、作用范围、短期指标和副作用记录 | 同类错误继续、旁路仍开放或副作用扩大 | 修复已生效 |
| `root_candidate_identified` | good/bad 边界、候选方法、依赖/配置假设和不确定性 | 复现不稳定、候选在真实条件下不成立 | 唯一根因 |
| `fix_candidate_built` | source/target commit、构建清单、制品摘要、测试和候选绑定 | 输入、依赖、权限或构建环境改变 | 已部署到所有受影响组件 |
| `release_approved` | 独立审批、发布引用、回退目标、数据库/消息兼容和 rollout 计划 | 审批/策略过期、制品或目标基线变化 | 运行已恢复 |
| `rollout_observing` | 实例实际 digest、流量版本、健康窗口、数据/消息观察 | 版本错绑、门槛超标、旧实例未围栏或证据不可见 | 可以关闭事故 |
| `resolved` | 机制相关指标、数据不变量、积压/重试、外部副作用和所有组件状态一致 | 新增同类错误、验证窗口不足或关键证据被撤销 | 复盘和长期行动已完成 |
| `closed` | 证据包、通知、残余风险接受、行动项 owner/截止和保留策略 | 新证据证明影响范围或根因判断错误 | 所有副本和风险绝对消失 |

状态记录应追加 `entered_at`、`entered_by`、`evidence_ids`、`blocked_reason` 和 `attempt`。例如，制品已经在 manager 上生效但 worker 仍运行旧 digest 时，状态可以是 `rollout_observing`，不能因为一个实例健康就写成 `resolved`；若数据库 contract 已完成，候选即使重新构建成功也要回到兼容性评估，而不是直接跳到 `release_approved`。
...
前置条件是发布基线和稳定分支已经冻结，修复范围、测试和变更审批已明确，工作区没有未保存内容。`cherry-pick` 会在目标分支创建新提交，可能冲突、产生空提交或改变上下文；冲突时保存状态，按[冲突与恢复](../part-07/13-local-and-remote-recovery.md)处理，不能跳过测试直接继续。来源提交和目标提交都要写入事故记录、构建清单和发布记录。
