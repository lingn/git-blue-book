...
性能比较最好使用成对样本：同一仓库快照、同一工作区、同一命令和同一机器先测基线，再只改变一个因素后重测。把不同 commit、不同依赖缓存或不同网络位置混在一起，得到的差异无法归因于优化本身。

至少把样本分成三组：

| 分组 | 目的 | 不能混入的因素 |
| --- | --- | --- |
| 冷启动/冷缓存 | 观察首次打开仓库、首次解包或首次按需取对象的成本 | 热缓存运行、后台预热、前一轮生成的对象 |
| 热缓存/稳定交互 | 观察开发者重复执行命令的常态 | 网络抖动、维护任务、IDE 扫描 |
| 异常或尾部 | 观察锁等待、重试、超时和失败 | 只保留成功样本后再计算平均值 |

冷缓存不等于“今天第一次运行”。操作系统页缓存、杀毒软件、文件索引器、容器层缓存和 Git 自动维护都可能让两次所谓冷启动不一致。真实冷缓存需要可重复的虚拟机或磁盘快照流程；不要在生产主机上用 `sudo purge`、drop caches 或重启制造数据点。

热缓存也要规定预热次数、等待时间和后台活动。至少报告 `n`、p50、p95、最大值、失败数和超时数。样本不足时直接列出原始样本，不写“提升了 30%”这类过度精确的结论。p50 改善而 p95 变差，通常说明少数锁、网络或大路径输入被放大，不能只报均值。

一次可复核的本地测量可以这样记录：

~~~bash
sample_file="$(mktemp "${TMPDIR:-/tmp}/git-perf-samples.XXXXXX")"
trap 'rm -f "$sample_file"' EXIT
for run in 1 2 3 4 5; do
  start_ns="$(python3 -c 'import time; print(time.time_ns())')"
  if git --no-optional-locks status --porcelain=v1 >/dev/null; then
    result=pass
  else
    result=fail
  fi
  end_ns="$(python3 -c 'import time; print(time.time_ns())')"
  printf '%s\t%s\t%s\n' "$run" "$result" "$((end_ns - start_ns))" \
    >> "$sample_file"
done
~~~

这段示例只适合已固定的测试副本，且依赖可用的 Python 3 作为纳秒计时器；也可以换成目标系统认可的计时工具。它会创建并写入临时 TSV，`status` 仍可能刷新 index 的 stat 信息。运行前清理会改变工作区的修改和锁状态，采集文件中不要写入秘密或完整内部路径。结果解析要保留失败行，不能用 `set -e` 让第一次失败直接丢掉整个样本集。

若生产环境禁止 Python 或临时文件写入，应使用组织批准的计时器，并在测量单中记录工具版本和输出处理。无论计时器怎样替换，样本的仓库 OID、缓存分组、并发负载和退出码都必须保持可追溯。
...
~~~text
workload: status --porcelain=v1
baseline: 热缓存 p95 <= 800 ms，失败率 = 0
candidate: p95 至少下降 15%，失败率不得上升
correctness: refs、HEAD tree、可达 commit 集和工作区不变
cost_guardrail: 维护峰值磁盘不超过剩余空间的 60%
rollback: 关闭辅助索引或恢复旧配置后可重测
~~~

阈值应来自用户等待时间、CI 超时或服务 SLO，而不是为了让实验“通过”临时选择。p95 下降但失败率、锁等待或磁盘峰值超过护栏时，结果是未通过。没有定义基线和护栏时，性能数据只能作为调查线索。

### 常见的错误归因

| 观察 | 可能的真正原因 | 下一步 |
| --- | --- | --- |
| 第一次 `status` 慢，后面很快 | index 或页缓存预热 | 分开冷/热样本，检查 Trace2，不先运行 gc |
| `clone` 变快但首次构建变慢 | partial clone 延迟水合 blob | 记录后续对象请求和总工作流时间 |
| p50 变快、p95 变差 | 锁争用、网络尾延迟或少数超大路径 | 分离并发和输入规模，保留异常样本 |
| pack 变小但 fetch 未变快 | 瓶颈在协商、代理或服务端限流 | 分解 client/server/transport 阶段 |
| commit-graph 文件生成后 `status` 不变 | 优化作用于历史遍历，不作用于工作区扫描 | 用 `log`、`merge-base` 等匹配 workload 复测 |
| 删除对象后磁盘下降 | 可能丢失不可达恢复点或外部 alternates | 先核对 refs、备份和恢复窗口，再决定清理 |

这些解释都需要规模指标和 Trace2 支持。单个“快了几秒”的截图不能证明因果，也不能成为删除对象、改变 clone 策略或拆分仓库的批准依据。
