...
> 最近更新：2026-09-06。状态只描述已经落盘并验证的工作。

## 当前里程碑

阶段 0 已完成。现有基础教程和实验保持可用，v2 第二篇补充、第七篇首批章节、第六篇与第八至十三篇已经落入权威路径，其余旧目录按逐章迁移表继续收束。当前共有 158 个公开页面、29 个兼容迁移页和 54 组隔离实验；正文尚未达到出版标准。
...
| 2026-09-06 | `./scripts/verify-all.sh` | 通过 | 第七篇共享历史政策、reflog 和本地/远程恢复迁移后整库回归，155 个公开页面、26 个兼容迁移页与 54 组隔离实验正常 |
...
- 第六篇的发布、CI 证据链、CI 触发与队列、受保护分支、bisect、历史检索、热修复、冲突报告、提交质量、排障、综合场景、stash 和 worktree 旧页，以及第五篇全部核心撤销/恢复入口、第四篇 rebase/cherry-pick 入口已改为兼容迁移页；第五篇综合练习和第七篇最终导航仍需继续审校，旧目录暂时不能移出导航。
