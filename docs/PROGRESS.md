...
阶段 0 已完成。现有基础教程和实验保持可用，v2 第二篇补充、第七篇首批章节、第六篇与第八至十三篇已经落入权威路径，其余旧目录按逐章迁移表继续收束。当前共有 149 个公开页面、19 个兼容迁移页和 54 组隔离实验；正文尚未达到出版标准。
...
- [ ] 继续迁移第五篇交互式 rebase、cherry-pick、revert、显式租约、reset、reflog 和恢复案例，处理与远端历史改写新增章节的职责重叠。
...
- 第六篇的发布、CI 证据链、CI 触发与队列、受保护分支、bisect、历史检索、热修复、冲突报告、提交质量、排障、综合场景、stash 和 worktree 旧页，以及第五篇前五个 amend/状态入口已改为兼容迁移页；第五篇交互式 rebase、cherry-pick、revert、显式租约、reset、reflog 和恢复案例仍需逐章迁移，旧目录暂时不能移出导航。
