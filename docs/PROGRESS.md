...
阶段 0 已完成。现有基础教程和实验保持可用，v2 第二篇补充、第六篇至第十三篇已经落入权威路径，其余旧目录按逐章迁移表继续收束。当前共有 160 个公开页面、32 个兼容迁移页和 54 组隔离实验；正文尚未达到出版标准。
...
- [ ] 继续审校交互式 rebase 与普通 rebase 工作流的冲突/中止/验证重复，以及共享政策、显式租约和远端改写事故的证据清单重复。
