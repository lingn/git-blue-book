...
> 最近更新：2026-09-04。状态只描述已经落盘并验证的工作。

## 当前里程碑

阶段 0 已完成。现有基础教程和实验保持可用，v2 第六篇与第八至十三篇已经落入权威路径，其余旧目录按逐章迁移表继续收束。当前共有 141 个公开页面、7 个兼容迁移页和 54 组隔离实验；正文尚未达到出版标准。
...
- 第六篇的发布、CI 证据链、CI 触发与队列、受保护分支、bisect、历史检索和热修复旧页已改为兼容迁移页；stash、worktree、冲突报告、提交质量、排障与综合案例仍等待按迁移表收束，旧目录暂时不能移出导航。
